...
import {getOrders} from '../helpers/dataListHelpers';
...
const ViewOrder = ({navigation}) => {
  ...
  const _printReciept = async item => {
    ...
    let initialValue= 0 
    // let sum= item.productprice.reduce(()=>{
    //   return (item.products.price*item.products.quantity) + (item.products.price*item.products.quantity)
    // },initialValue)
