...
const accept = async order_id => {
  ...
  try {
    let response = await axios.get(
      `http://143.110.244.110/tija/frontuser/accept?order_id=${order_id}`,
...
const cancle = async order_id => {
  let userToken = await AsyncStorage.getItem('token');
  try {
    let response = await axios.get(
      `http://143.110.244.110/tija/frontuser/cancle_order?order_id=${order_id}`,
...
export {accept, cancle, preparing, ready, delivered};
